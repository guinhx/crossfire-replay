...
public static class LtMessageReader
{
    ...
    private static int GetMaxPlausiblePayloadBytes(EMessageId id) => id switch
    {
        ...
        EMessageId.MsgScAiScore => 64,
        EMessageId.MsgCsRappelVelAndRot => 256,
    ...
    public static LtDecodedMessage? TryDecode(ReadOnlySpan<byte> payload)
    {
        ...
        try
        {
            return id switch
            {
                ...
                EMessageId.MsgCsFirstUpdate => LtCsSemanticDecoders.TryDecodeFirstUpdate(payload) ?? new LtUnknownDecoded(id, payload.Length),
                EMessageId.MsgCsRappelVelAndRot => LtCsSemanticDecoders.TryDecodeRappelVelAndRot(payload) ?? new LtUnknownDecoded(id, payload.Length),
                EMessageId.MsgScAiAckCanDefuseC4 => LtScReplayDecoders.TryDecodeAiAckCanDefuseC4(payload) ?? new LtUnknownDecoded(id, payload.Length),
                EMessageId.MsgScAiScore => LtScReplayDecoders.TryDecodeAiScore(payload) ?? new LtUnknownDecoded(id, payload.Length),
                EMessageId.MsgScDamageCalculationRequest => LtScReplayDecoders.TryDecodeDamageCalculationRequest(payload) ?? new LtUnknownDecoded(id, payload.Length),
                EMessageId.MsgScSheepWantedList => LtScReplayDecoders.TryDecodeSheepWantedList(payload) ?? new LtUnknownDecoded(id, payload.Length),
                EMessageId.MsgScPresentTeamAceUser => LtScReplayDecoders.TryDecodePresentTeamAceUser(payload) ?? new LtUnknownDecoded(id, payload.Length),
